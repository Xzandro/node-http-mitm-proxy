  ...
  */
  if (head[0] == 0x16 || head[0] == 0x80 || head[0] == 0x00) {
    ...
    sem.take(function() {
      ...
      getHttpsServer(hostname, req, function(err, port) {
        process.nextTick(sem.leave.bind(sem));
        if (err) {
  ...
  function getHttpsServer(hostname, req, callback) {
    self.onCertificateRequired(hostname, req, function (err, files) {
      if (err) {
        return callback(err);
      }
      async.auto({
...
Proxy.prototype.onCertificateRequired = function (hostname, req, callback) {
  var self = this;
  return callback(null, {
